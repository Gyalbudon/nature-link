...
class Send {
  ...
  public function run($adminMail, $adminName, $returnMailTitle, $returnMailHeader, $returnMailFooter, $returnURL) {
    ...
    if (!isset($_SESSION["submitFile"])) {
      $this->_adminSend($returnURL,null, $server);
    }else{
  ...
  private function _adminSend($returnURL, $submitFile, $server) {
    // 送信先の設定
    ...
    // タイトルの設定
    $sendTitle = "ページにお問い合わせがありました";
    $sendTitle = mb_encode_mimeheader($sendTitle, "ISO-2022-JP-MS","UTF-8");

    // メッセージの設定
    $this->sendMessage = "ホームページ ";
    $this->sendMessage .= $returnURL;
    $this->sendMessage .= "  に\n";
    $this->sendMessage .= "お客様からお問い合わせがありました。\n";
    $this->sendMessage .= "※本メールは、プログラムから自動で送信しています。\n\n";
    $this->sendMessage .= "以下お客様情報です。\n";
    $this->sendMessage .= "−−−−−−−−−−−−−−−−−−−−−−−−−−\n";
    ...
    $this->sendMessage .= "−−−−−−−−−−−−−−−−−−−−−−−−−−\n\n";
    $this->sendMessage .= "お客様への折り返しのご連絡を宜しくお願い致します。\n";
    $this->sendMessage .= "\n\n";
